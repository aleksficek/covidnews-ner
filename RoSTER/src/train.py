...
def main():
    ...
    # Custom parameters
    parser.add_argument("--backbone_dir",
                        default="",
                        type=str,
                        help="directory to backbone")
    parser.add_argument("--backbone_dir_s1",
                        default="",
                        type=str,
                        help="directory to backbone")
    parser.add_argument("--save_backbone",
                        action='store_true',
                        help="whether to save backbone")
    parser.add_argument("--save_model",
                        action='store_true',
                        help="whether to save model")
    parser.add_argument("--do_strong_tuning",
                        action='store_true',
                        help="whether to only do strong data tuning on saved model.")
    parser.add_argument("--do_strong_tuning_eval",
                        action='store_true',
                        help="whether to only do strong data tuning on saved model.")
    ...
    if args.do_strong_tuning:
        trainer = RoSTERTrainer(args)
        trainer.load_model("final_model.pt", args.backbone_dir_s1)
    ...
    if args.do_strong_tuning_eval:
        print("Strong tuning eval flag set")
        # trainer = RoSTERTrainer(args)
