...
class RoSTERTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.save_the_model = args.save_model
        self.save_backbone = args.save_backbone
        self.backbone_dir = args.backbone_dir
        ...
        if args.do_train or args.do_strong_tuning:
        ...
        if args.do_eval or args.do_strong_tuning_eval:
    ...
    def noise_robust_train(self, model_idx=0):
        ...
        # If model exists than use it
        # if os.path.exists(os.path.join(self.backbone_dir, f"training_model_{model_idx}.pt")):
        if os.path.exists(os.path.join(self.backbone_dir, f"final_model.pt")):
            print(f"\n\n******* Model {model_idx} backbone found! Using it *******\n\n")
            model = self.model.to(self.device)
            model.load_state_dict(torch.load(os.path.join(self.backbone_dir, f"final_model.pt")))
        else:
            print(f"\n\n******* Could not find model backbone *******\n\n")
        ...
        if self.save_backbone:
            torch.save(model.state_dict(), os.path.join(self.output_dir, f"training_model_{model_idx}.pt"))
    ...
    def update_weights(self, model):
        ...
        for i in range(1, self.num_labels):
            ...
            remove_type_frac = remove_type_num / max(type_label_pos.sum().item(), 1)
